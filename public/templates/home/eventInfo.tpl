<div class='eventInfo'>
	{if $event.closed}
	  <div class='eventClosedMsg'>
		...
		<p><strong>Cerrado</strong> por {$event.closedBy} ({$event.closedOn|date_format:'d/m/Y, H:i'})</p>
		<p><i>{$event.closed}</i></p>
	  </div>
	{/if}

	<div class='eventInfoTitle'>{$event.ini|date_format:'l, d \d\e F \d\e Y'}</div>
	<div class='eventInfoSubTitle'>{$event.ini|date_format:'H:i'}{if $event.end} - {$event.end|date_format:'H:i'}{/if}</div>
	...
	<table class='eventInfoTbl' cellpadding="3" cellspacing="0">
		...
		<tr>
			<th>Agendado para</th>
			<td>
				{$event.ini|date_format:'d/m/Y, H:i'}
		...
		<tr>
			...
			<td>
				...
				({$event.created|date_format:'d/m/Y, H:i'})
			</td>
		</tr>
		{if $editions}
		  <tr>
			...
			<td>
			  {foreach from=$editions item=edition}
				...
				({$edition.on|date_format:'d/m/Y, H:i'})<br />
			  {/foreach}
			</td>
		  </tr>
		{/if}
	...
	{if Access::can('editEvent') && $event.canEdit}
