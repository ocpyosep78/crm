 ...
 */
	...
	class Nav{
		...
		public function getPage($page, $atts=array(), $msg='', $type=0, $inFrame=false){
			...
			if( $code === true ){
				loadMainSmartyVars();
				require_once(CORE_PATH.'pageMgr.php');
		...
		/**
		 * Loads a page within an iframe (core/frame.tpl)
		 */
		...
		public function queueMsg($msg='', $type=0){
			...
			$_SESSION['queuedMsg'] = array('msg' => $msg, 'type' => $type);
		}
		...
		public function processQueuedMsg(){
			if( !isset($_SESSION['queuedMsg']) ) $this->queueMsg();
