...
text=text.replace(/</g,"&lt;")
text=text.replace(/>/g,"&gt;")
debugTag=this.debugWindow.$('debugTag');debugTag.innerHTML=('<b>'+(new Date()).toString()+'</b>: '+text+'<hr/>')+debugTag.innerHTML;}catch(e){alert("Xajax Debug:\n "+text);}
...
this.objectToXML=function(obj){var sXml="<xjxobj>";for(i in obj){try{if(!obj.hasOwnProperty(i))continue;if(i=='constructor')
...
this.loadingFunction=function(){};this.doneLoadingFunction=function(){};this.loadingFailedFunction=function(){};var loadingTimeout;this.call=function(sFunction,aArgs,sRequestType){var i,r,postData;if(document.body&&xajaxWaitCursor)
document.body.style.cursor='wait';if(xajaxStatusMessages==true)window.status='Sending Request...';clearTimeout(loadingTimeout);loadingTimeout=setTimeout(xajax.loadingFunction, 300);if(xajaxDebug)this.DebugMessage("Starting xajax...");if(sRequestType==null){var xajaxRequestType=xajaxDefinedPost;}
...
r.onreadystatechange=function(){
	...
	if(r.status == 200){
		...
		if(r.responseXML && r.responseXML.documentElement) xajax.processResponse( r.responseXML );
			...
			xajax.loadingFailedFunction();
			trimmedResponseText = r.responseText.replace(/^\s+/g, "").replace(/\s+$/g, "");
			if( xajaxVerbose ){
				var errorString = "Error: the XML response that was returned from the server is invalid." +
					"\nReceived:\n" + r.responseText +
					((trimmedResponseText != r.responseText) ? '\nYou have whitespace in your response.' : '');
				alert( errorString );
			};
		...
		if( xajax.responseErrorsForAlert.containsValue(r.status) ){
			...
			errorString += "\nReceived:\n" + r.responseText;
			if( xajaxVerbose ) alert( errorString );
		}
		if(xajaxStatusMessages == true) window.status='Invalid XML response error';
	}
...
else if(xml.childNodes[i].firstChild)data=xml.childNodes[i].firstChild.nodeValue;else
...
if(cmd=="al"){cmdFullname="addAlert";alert(data);}
	...
	if( typeof(data) !== 'object' ) throw('Xajax addScriptCall error: wrong datatype');
	cmdFullname = "addScriptCall";
	var aPath = id.split('.');
	var oObj = window[aPath.shift()];
