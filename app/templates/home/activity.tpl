...
{foreach from=$events item=event}
  <div class='activity_item'>
	<div>
	  <strong>Fecha</strong>: <span class='link2model' for='agendaDay|{$event.ini|date_format:'Y/m/d'}'>{$event.ini|date_format:'Y-m-d'}</span> | <strong>Hora</strong>: {$event.ini|date_format:'H:i'} (ingresado por <span class='link2model' for='usersInfo|{$event.creator}'>{$event.creator}</span>{if $event.target} para <span class='link2model' for='usersInfo|{$event.target}'>{$event.target}</span>{/if})
	...
	<div class='linkLike' onclick="eventInfo('{$event.uid}');">Ver evento</div>
  </div>
{foreachelse}
...
{foreach from=$notes item=note}
  <div for='{$note.uid}' class='activity_item'>
	<div>
	  <strong>Fecha</strong>: {$note.date|date_format:'Y-m-d'} | <strong>Hora</strong>: {$note.date|date_format:'H:i'} (ingresado por <span class='link2model' for='usersInfo|{$note.by}'>{$note.by}</span>{if $note.user} para <span class='link2model' for='usersInfo|{$note.user}'>{$note.user}</span>{/if})
