...
function editUsers($data)
{
	...
	else return FileForm::addResponse("showTip('editUsers_{$valid['field']}', '{$valid['tip']}');");
...
function closeAgendaEvent($id, $msg, $rescheduled=false)
{
	...
	if( $ans->error ) return say('Ocurrió un error al intentar cerrar el evento.');
	else{
...
function createNotes($data, $modifier){
	...
	else return addScript("showTip('{$pfx}{$valid['field']}', '{$valid['tip']}');");
